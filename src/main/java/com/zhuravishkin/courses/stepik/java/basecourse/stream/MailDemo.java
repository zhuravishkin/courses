package com.zhuravishkin.courses.stepik.java.basecourse.stream;
...
//   где по получателю можно получить список зарплат, которые были ему отправлены.
    ...
    public static class MailService<T> implements Consumer<Message<T>> {
        private final Map<String, List<T>> mailBox = new GetMap<>();

        private static class GetMap<K, V> extends HashMap<K, V> {
            @Override
            public V get(Object key) {
                V v = super.get(key);
                if (v == null) {
                    v = (V) Collections.emptyList();
                }
                return v;
            }
        }
        ...
        public void accept(Message<T> tMessage) {
            if (mailBox.containsKey(tMessage.getTo())) {
                List<T> list = mailBox.get(tMessage.getTo());
